...
namespace JaLoaderUnity4
{
    public class DebugCamera : MonoBehaviour
    {
        ...
        private void OnGameLoad()
        {
            ...
            createdInGamePPCamera = cameraObj.transform.GetChild(0).gameObject.GetComponent<BloomAndLensFlares>();
            ...
            if (!createdInGamePPCamera)
            {
                ...
                foreach(MonoBehaviour behaviour in cameraObj.transform.GetChild(0).GetComponents<MonoBehaviour>())
                {
                    // if not Camera or GUILayer or BloomAndLensFlares or SunShafts or GlobalFog or Vignetting or SSAOPro or AntiAliasingAsScreenEffect, destroy it
                    if (behaviour.GetType() != typeof(Camera) && behaviour.GetType() != typeof(GUILayer) && behaviour.GetType() != typeof(BloomAndLensFlares) && behaviour.GetType() != typeof(SunShafts) && behaviour.GetType() != typeof(GlobalFog) && behaviour.GetType() != typeof(Vignetting) && behaviour.GetType() != typeof(SSAOPro) && behaviour.GetType() != typeof(AntialiasingAsPostEffect))
                    {
                        Destroy(behaviour);
                    }
                }
        ...
        void Update()
        {
            ...
            {
                if (Application.loadedLevel == 3 && firstTimeOpening)
                {
